% The orignal script takes as input a filename (in the variable fname) that
...
% Modified on 2 April 2020 by Omama Elrefaei
% The modified function  show the numerical solutions and performs an AC analysis too.

function [A, x, z, na, nx, nz, enx, mySys]= netlist_fun(fname, first_node, second_node)
    ...
    A = str2sym([G B; C D]); %Create A matrix
    x=str2sym([v;j]);       %Create x vector
    z=str2sym([i;e]);       %Create z vector
    ...
    %The transfer function between two nodes netlist:
    equationsOfunknowns = x==enx;
    H = equationsOfunknowns(first_node)/equationsOfunknowns(second_node);
    Hnumbers = rhs(H);
    [n,d]=numden(Hnumbers);
